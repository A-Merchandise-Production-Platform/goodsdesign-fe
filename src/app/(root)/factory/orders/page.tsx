...
import { Clock, ArrowRight, CheckCircle, AlertCircle, Loader2, XCircle } from 'lucide-react'
...
export default function FactoryOrdersPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      ...
      case 'IN_PRODUCTION':
        return <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200"><Loader2 className="w-3 h-3 mr-1" /> In Production</Badge>
      case "REJECTED":
        return (
  ...
  const orders = data?.factoryOrdersByFactory
              ...
              <TableBody>
                {orders.map((order) => (
                  <TableRow key={order.id}>
                    ...
                    <TableCell>{formatDate(order.acceptanceDeadline)}</TableCell>
