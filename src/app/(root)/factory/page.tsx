...
import { AlertTriangle, Clock, DollarSign, Hammer, Package } from "lucide-react"
...
export default function FactoryDashboard() {
  ...
  return (
    <DashboardShell title="Factory Dashboard" subtitle="Monitor production and order status">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{loading ? "Loading..." : dashboard?.pendingOrders || 0}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{loading ? "Loading..." : dashboard?.inProductionOrders || 0}</div>
            <p className="text-xs text-muted-foreground">Currently being manufactured</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            {loading ? (
              ...
              <div className="space-y-8">
                {dashboard.productionProgress.map((order) => {
                  const latestReport = order.progressReports?.[order.progressReports.length - 1]
                  const progress = latestReport ? calculateProgress(latestReport) : 0
                    ...
                    <div key={order.id} className="space-y-2">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            Customer: {order.customerOrder?.customer?.name || "N/A"}
                          </p>
                        </div>
                        <Badge variant={getStatusVariant(order.status)}>{order.status}</Badge>
                      </div>
                      ...
                      <div className="flex justify-between text-xs text-muted-foreground">
                        ...
                        {latestReport?.estimatedCompletion && (
                          <span>Est. Completion: {formatDate(latestReport.estimatedCompletion)}</span>
                        )}
                      </div>
                      {latestReport?.notes && (
                        <p className="text-xs italic border-l-2 pl-2 mt-1 border-muted">"{latestReport.notes}"</p>
                      )}
                    </div>
                  )
                })}
              ...
              <div className="text-center py-4">No production in progress</div>
            )}
          </CardContent>
        </Card>
      </div>
