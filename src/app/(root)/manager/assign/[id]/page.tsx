          ...
          <CardContent className="pt-6">
            <div className="flex flex-col items-center justify-center h-64">
              ...
              <p className="text-muted-foreground mb-4">
                The factory order you're looking for doesn't exist or has been removed.
              </p>
      ...
      <div className="grid gap-6">
        <Card>
          <CardHeader>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <CardTitle className="text-2xl">Factory Order: {currentOrder?.id}</CardTitle>
                <CardDescription>Customer Order: {currentOrder?.customerOrder?.id || "N/A"}</CardDescription>
              </div>
              <Badge className="w-fit" variant={getStatusBadgeVariant(currentOrder?.status)}>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="flex flex-col space-y-1">
                ...
                <span className="font-medium">{formatDate(currentOrder?.acceptanceDeadline)}</span>
              </div>
              <div className="flex flex-col space-y-1">
                <span className="text-sm font-medium text-muted-foreground flex items-center">
                ...
                <span className="font-medium">{currentOrder?.totalItems || 0}</span>
              </div>
              <div className="flex flex-col space-y-1">
                <span className="text-sm font-medium text-muted-foreground flex items-center">
                ...
                <span className="font-medium">${currentOrder?.totalProductionCost?.toFixed(2) || "0.00"}</span>
              </div>
            </div>
            ...
            {currentOrder?.isDelayed && (
              <div className="mt-6 p-4 border border-red-200 bg-red-50 rounded-md flex items-start">
                ...
                <div>
                  ...
                  <p className="text-sm text-red-600">{currentOrder?.delayReason || "No reason provided"}</p>
                </div>
              </div>
            )}

            {typeof currentOrder?.currentProgress === 'number' && currentOrder.currentProgress > 0 && (
              <div className="mt-6">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-medium">Production Progress</span>
                  <span className="text-sm font-medium">{currentOrder?.currentProgress}%</span>
                </div>
                <Progress value={currentOrder?.currentProgress} className="h-2" />
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="flex flex-col space-y-1">
                <span className="text-sm font-medium text-muted-foreground">Assigned At</span>
                <span className="font-medium">{formatDate(currentOrder?.assignedAt)}</span>
              </div>
              <div className="flex flex-col space-y-1">
                <span className="text-sm font-medium text-muted-foreground">Accepted At</span>
                <span className="font-medium">{formatDate(currentOrder?.acceptedAt)}</span>
              </div>
              <div className="flex flex-col space-y-1">
                <span className="text-sm font-medium text-muted-foreground">Completed At</span>
                <span className="font-medium">{formatDate(currentOrder?.completedAt)}</span>
              </div>
              <div className="flex flex-col space-y-1">
                <span className="text-sm font-medium text-muted-foreground">Estimated Completion</span>
                <span className="font-medium">{formatDate(currentOrder?.estimatedCompletionDate)}</span>
              </div>
            </div>
            ...
            <div className="flex justify-end">
              <Dialog open={assignDialogOpen} onOpenChange={setAssignDialogOpen}>
                <DialogTrigger asChild>
                  <Button>
                    {currentOrder?.status === "WAITING_FOR_MANAGER_ASSIGN_FACTORY"
                      ? "Assign to Factory"
                      : "Assign to Staff"}
                  </Button>
                </DialogTrigger>
        ...
        <Tabs defaultValue="order-details">
          <TabsList className="grid w-full grid-cols-4">
            ...
            <TabsTrigger value="rejection-history">Rejection History</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="order-details">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Details of items included in this factory order</CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
                  ...
                  <TableBody>
                    {currentOrder?.orderDetails && currentOrder.orderDetails.length > 0 ? (
                      currentOrder.orderDetails.map((detail) => (
                        <TableRow key={detail?.id}>
                          <TableCell className="font-medium">{detail?.id}</TableCell>
                          <TableCell>{detail?.quantity}</TableCell>
                          <TableCell>${detail?.productionCost?.toFixed(2) || "0.00"}</TableCell>
                          <TableCell>
                            ...
                            <Badge
                              variant={
                                (detail?.qualityStatus as string) === "PASSED"
                                  ? "default"
                                  : (detail?.qualityStatus as string) === "FAILED"
                                    ? "destructive"
                                    : "outline"
                              }
                            >
                        ...
                        {currentOrder.orderDetails.map((detail, index) =>
                          detail?.checkQualities && detail.checkQualities.length > 0 ? (
                            <AccordionItem key={detail?.id} value={`item-${index}`}>
                              <AccordionTrigger>
                                Item {detail?.id} Quality Checks ({detail.checkQualities.length})
                              </AccordionTrigger>
                              <AccordionContent>
                                <Table>
                                  <TableHeader>
                                  ...
                                  <TableBody>
                                    {detail.checkQualities.map((check) => (
                                      <TableRow key={check?.id}>
                                        <TableCell>{check?.checkedBy || "N/A"}</TableCell>
                                        <TableCell>{formatDate(check?.checkedAt)}</TableCell>
                                        <TableCell>{check?.passedQuantity || 0}</TableCell>
                                        <TableCell>{check?.failedQuantity || 0}</TableCell>
                                        <TableCell>
                                          <Badge
                                            variant={
          ...
          <TabsContent value="design-info">
            <Card>
              ...
              <CardContent>
                {currentOrder?.customerOrder?.orderDetails?.map((detail, index) => (
                  <div key={index} className="mb-6">
                    <h3 className="text-lg font-semibold mb-2">Design #{index + 1}</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {detail.design?.designPositions?.map((position, posIndex) => (
                        <div key={posIndex} className="border rounded-md p-4">
                          <h4 className="font-medium mb-2">
                            Position: {position.positionType?.positionName || "Unknown"}
                          </h4>
                          <p className="text-sm text-muted-foreground mb-2">Design ID: {position.designId}</p>
                          <p className="text-sm text-muted-foreground">
                            Position Type ID: {position.productPositionTypeId}
                          </p>
                        </div>
                      ))}
                    </div>
          ...
          <TabsContent value="progress-reports">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Production progress updates from the factory</CardDescription>
              </CardHeader>
              <CardContent>
                {currentOrder?.progressReports && currentOrder.progressReports.length > 0 ? (
                      ...
                      <div key={report?.id} className="border rounded-md p-4">
                        <div className="flex flex-col md:flex-row md:items-center justify-between mb-4">
                          <div className="flex items-center">
                            ...
                            <h3 className="font-medium">Report from {formatDate(report?.reportDate)}</h3>
                          </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                          <div>
                            <p className="text-sm font-medium text-muted-foreground mb-1">Estimated Completion</p>
          ...
          <TabsContent value="rejection-history">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Factories that have rejected this order</CardDescription>
              </CardHeader>
              <CardContent>
                {currentOrder?.rejectedHistory && currentOrder.rejectedHistory.length > 0 ? (
                  <div className="space-y-6">
                    {currentOrder.rejectedHistory.map((rejection) => (
                      <div key={rejection?.id} className="border rounded-md p-4">
                        <div className="flex flex-col md:flex-row md:items-center justify-between mb-4">
                          <div className="flex items-center">
                            <Factory className="h-5 w-5 text-muted-foreground mr-2" />
                            <h3 className="font-medium">{rejection?.factory?.name}</h3>
                          </div>
                          <Badge variant="outline" className="mt-2 md:mt-0">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                          <div>
                            ...
                            <p>
                              {rejection?.factory?.contactPersonName} ({rejection?.factory?.contactPersonRole})
                            </p>
                            <p className="text-sm">{rejection?.factory?.contactPersonPhone}</p>
                          </div>
                          <div>
                            <p className="text-sm font-medium text-muted-foreground mb-1">Factory Owner</p>
                            <div className="flex items-center">
                              <Avatar className="h-6 w-6 mr-2">
                                <AvatarImage
                                  src={rejection?.factory?.owner?.imageUrl || ""}
                                  alt={rejection?.factory?.owner?.name || ""}
                                />
                                <AvatarFallback>{rejection?.factory?.owner?.name?.charAt(0) || "?"}</AvatarFallback>
                              </Avatar>
                              <div>
                                <p>{rejection?.factory?.owner?.name}</p>
                                <p className="text-sm text-muted-foreground">{rejection?.factory?.owner?.email}</p>
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="mt-4">
                          <p className="text-sm font-medium text-muted-foreground mb-1">Factory Details</p>
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                            <div>
                              <p className="text-sm text-muted-foreground">Lead Time</p>
                              <p className="text-sm">{rejection?.factory?.leadTime} days</p>
                            </div>
                            <div>
                              <p className="text-sm text-muted-foreground">Capacity</p>
                              <p className="text-sm">{rejection?.factory?.maxPrintingCapacity} units</p>
                            </div>
                            <div>
                              <p className="text-sm text-muted-foreground">Min. Order</p>
                              <p className="text-sm">{rejection?.factory?.minimumOrderQuantity} units</p>
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-muted-foreground">No rejection history available</p>
                  </div>
                )}
              </CardContent>
            </Card>
